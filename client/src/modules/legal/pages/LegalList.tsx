...
import { Edit, Trash2, Scale, ShieldCheck, Download, ChevronDown } from 'lucide-react';
...
export default function LegalList() {
  ...
  const handleExportCSV = async () => {
    setExporting(true);
    try {
      const isReviews = store.activeTab === 'reviews';
      const { data } = isReviews
        ? await legalApi.listReviews({ limit: 1000 })
        : await legalApi.listCompliance({ limit: 1000 });
      const allItems = data.data || data.items || [];
      const label = isReviews ? 'reviews' : 'compliance';
      exportToCSV(allItems, `legal-${label}-export`);
  ...
  const handleExportJSON = async () => {
    ...
    try {
      ...
      const allItems = data.data || data.items || [];
      const label = isReviews ? 'reviews' : 'compliance';
      exportToJSON(allItems, `legal-${label}-export`);
