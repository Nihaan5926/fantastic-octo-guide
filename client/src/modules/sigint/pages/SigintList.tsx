...
export default function SigintList() {
  ...
  const handleExportCSV = async () => {
    setExporting(true);
    try {
      const isIntercepts = activeTab === 'intercepts';
      const { data } = isIntercepts
        ? await sigintApi.listIntercepts({ limit: 1000 })
        : await sigintApi.listEmitters({ limit: 1000 });
      const allItems = data.data || data.items || [];
      const label = isIntercepts ? 'intercepts' : 'emitters';
      exportToCSV(allItems, `sigint-${label}-export`);
  ...
  const handleExportJSON = async () => {
    ...
    try {
      ...
      const allItems = data.data || data.items || [];
      const label = isIntercepts ? 'intercepts' : 'emitters';
      exportToJSON(allItems, `sigint-${label}-export`);
